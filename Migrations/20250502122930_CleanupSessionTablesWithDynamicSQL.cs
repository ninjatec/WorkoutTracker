...
{
    /// <inheritdoc />
    public partial class CleanupSessionTablesWithDynamicSQL : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Dynamic SQL to drop all indexes and foreign keys related to Rep.SetsSetId
            migrationBuilder.Sql(@"
                -- First, drop any indexes that depend on the SetsSetId column in Rep table
                ...
                -- Next, drop any foreign keys that reference the Set and Session tables
                SET @sql = '';
                SELECT @sql += 'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(t.schema_id)) + '.' + QUOTENAME(OBJECT_NAME(f.parent_object_id)) + 
                ...
                -- Also drop any foreign keys on Rep that reference the SetsSetId column
                SET @sql = '';
                SELECT @sql += 'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(t.schema_id)) + '.' + QUOTENAME(OBJECT_NAME(f.parent_object_id)) + 
                               ' DROP CONSTRAINT ' + QUOTENAME(f.name) + ';' + CHAR(13)
                FROM sys.foreign_keys f
                ...
                WHERE OBJECT_NAME(f.parent_object_id) = 'Rep'
                AND c.name = 'SetsSetId';
